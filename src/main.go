...
func initialize() {
  ...
  bucket := storageClient.Bucket(gcsBucketName)
	...
	for ittr, key := range initResponse.KeysBase64 {
    ...
    var bucketObj string
		bucketObj = strings.Join(string(ittr), "unseal-keys.json.enc")

		unsealKeysObject := bucket.Object(bucketObj).NewWriter(ctx)
		...
		log.Printf("Unseal keys written to gs://%s/%s", gcsBucketName, bucketObj)
	}
